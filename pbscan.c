/*
 ...
 Copyright (C) 2017 Tuomas Hamala
...
Geno_s *readVcf(FILE *geno_file, char **pop1, char **pop2, char **pop3, char name[], int indiv_n, int *snp_i, int *poplist, int print){
	...
	int i=0, j=0, line_i=0, char_i=0, maxchar=0, split_i=0, indiv_i=0, pop_i=0, id_ok=0, *tempind;
	char c, *line, *temp=NULL, *p=NULL, **templist, namec[30];
    const char uph[2]="/", ph[2]="|";
    Geno_s *sites;
    FILE *pbs_file;
    ...
    if(print == 1){
        ...
        if((pbs_file = fopen(namec, "w")) == NULL){
            printf("\nWarning: Cannot create file %s\n\n", namec);
            print = 0;
        }
        else
            fprintf(pbs_file, "CHR\tBP\t");
    }
	...
	while(fgets(line, maxchar+5, geno_file) != NULL){
        ...
        if(strcmp(temp,"#CHROM") == 0 && temp != NULL){
            ...
            if(print == 1)
                fprintf(pbs_file, "%s\t", sites[*snp_i].chromo);
			...
			while(temp != NULL){
                ...
                if(split_i == 1){
                    ...
                    if(print == 1)
                        fprintf(pbs_file, "%i\t", sites[*snp_i].position);
			...
			for(i=0;i<indiv_i;i++){
                ...
                if(p != NULL)
                    temp = strtok(templist[i], uph);
                else
                    temp = strtok(templist[i], ph);
                
                if(isdigit(temp[0]))
                    sites[*snp_i].genolist[i][0] = atoi(temp);
                
                if(p != NULL)
                    temp = strtok(NULL, uph);
                else
                    temp = strtok(NULL, ph);
                ...
                if(print == 1){
                    ...
                    if(sites[*snp_i].genolist[i][0] == -1)
                        ...
                        fprintf(pbs_file, "%i%i\t", sites[*snp_i].genolist[i][0], sites[*snp_i].genolist[i][1]);
                }
			}
...
Pbs_s **estimatePBS(Geno_s *sites, int poplist[], double **ms, int sims, int snp_n, int indiv_n, int win, int step, int perm, int div, int min, int like, double maf){
    ...
    double t_div[3]={0}, pbs[3]={0}, fst_ms[3]={0}, **pbs_ms;
...
void showHelp(void){
    ...
    puts("Copyright (C) 2017 Tuomas Hamala\n");
